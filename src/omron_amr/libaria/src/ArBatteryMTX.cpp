...
AREXPORT ArBatteryMTX::ArBatteryMTX(int batteryBoardNum, const char *name,
									...
									ArRobot *robot) : mySensorInterpTask(this, &ArBatteryMTX::sensorInterp),
													  myConn(conn),
													  myName(name),
...
AREXPORT bool ArBatteryMTX::blockingConnect(bool sendTracking, bool recvTracking)
{
	...
	if (myConn->getStatus() != ArDeviceConnection::STATUS_OPEN && !myConn->openSimple())
	{
		ArLog::log(
			ArLog::Terse,
...
AREXPORT bool ArBatteryMTX::sendPowerOff()
{
	...
	IFDEBUG(
		ArLog::log(ArLog::Normal,
				   "%s::sendPowerOff() Power Off request sent to Battery", getName());); // end IFDEBUG
...
AREXPORT bool ArBatteryMTX::sendPowerOffCancel()
{
	...
	IFDEBUG(
		ArLog::log(ArLog::Normal,
				   "%s::sendPowerOffCancel() Power Off Cancel request sent to Battery", getName());); // end IFDEBUG
...
AREXPORT bool ArBatteryMTX::sendStopCharging()
{
	...
	IFDEBUG(
		ArLog::log(ArLog::Normal,
				   "%s::sendStopCharging() Stop Charging request sent to Battery", getName());); // end IFDEBUG
...
AREXPORT bool ArBatteryMTX::sendStartCharging()
{
	...
	IFDEBUG(
		ArLog::log(ArLog::Normal,
				   "%s::sendStartCharging() Start Charging request sent to Battery", getName());); // end IFDEBUG
...
AREXPORT bool ArBatteryMTX::sendSetPowerOffDelay(unsigned int msDelay)
{
	...
	IFDEBUG(
		ArLog::log(ArLog::Normal,
				   "%s::sendSetPowerOffDelay() Set Power Off Delay sent to Battery", getName());); // end IFDEBUG
...
AREXPORT bool ArBatteryMTX::sendSetRealTimeClock(unsigned int secSinceEpoch)
{
	...
	IFDEBUG(
		ArLog::log(ArLog::Normal,
				   "%s::sendSetRealTimeClock() Set Real Time Clock sent to Battery", getName());); // end IFDEBUG
...
AREXPORT bool ArBatteryMTX::sendResetCellData()
{
	...
	IFDEBUG(
		ArLog::log(ArLog::Normal,
				   "%s::sendResetCellData() Reset Cell Data sent to Battery", getName());); // end IFDEBUG
...
AREXPORT bool ArBatteryMTX::sendSetReserveValue(unsigned short hundredthOfPercent)
{
	...
	IFDEBUG(
		ArLog::log(ArLog::Normal,
				   "%s::sendSetReserveValue() Set Reserve Value sent to Battery", getName());); // end IFDEBUG
...
AREXPORT bool ArBatteryMTX::sendSetBalanceValue(unsigned short hundredthOfPercent)
{
	...
	IFDEBUG(
		ArLog::log(ArLog::Normal,
				   "%s::sendSetBalanceValue() Set Balance Value sent to Battery", getName());); // end IFDEBUG
...
AREXPORT bool ArBatteryMTX::sendEmergencyPowerOff()
{
	...
	IFDEBUG(
		ArLog::log(ArLog::Normal,
				   "%s::sendEmergencyPowerOff() Emergency Off sent to Battery", getName());); // end IFDEBUG
...
AREXPORT void ArBatteryMTX::updateSystemInfo(unsigned char *buf)
{
	...
	mySerialNumber =
		(static_cast<unsigned int>(buf[6]) << 24) |
		(static_cast<unsigned int>(buf[5]) << 16) |
		(static_cast<unsigned int>(buf[4]) << 8) |
		static_cast<unsigned int>(buf[3]);
	myCurrentTime =
		(static_cast<unsigned long long>(buf[14]) << 56) |
		(static_cast<unsigned long long>(buf[13]) << 48) |
		(static_cast<unsigned long long>(buf[12]) << 40) |
		(static_cast<unsigned long long>(buf[11]) << 32) |
		(static_cast<unsigned long long>(buf[10]) << 24) |
		(static_cast<unsigned long long>(buf[9]) << 16) |
		(static_cast<unsigned long long>(buf[8]) << 8) |
		static_cast<unsigned long long>(buf[7]);
	myLastChargeTime =
		(static_cast<unsigned long long>(buf[22]) << 56) |
		(static_cast<unsigned long long>(buf[21]) << 48) |
		(static_cast<unsigned long long>(buf[20]) << 40) |
		(static_cast<unsigned long long>(buf[19]) << 32) |
		(static_cast<unsigned long long>(buf[18]) << 24) |
		(static_cast<unsigned long long>(buf[17]) << 16) |
		(static_cast<unsigned long long>(buf[16]) << 8) |
		static_cast<unsigned long long>(buf[15]);
	myChargeRemainingEstimate =
		(static_cast<unsigned int>(buf[26]) << 24) |
		(static_cast<unsigned int>(buf[25]) << 16) |
		(static_cast<unsigned int>(buf[24]) << 8) |
		static_cast<unsigned int>(buf[23]);
	myCapacityEstimate =
		(static_cast<unsigned int>(buf[30]) << 24) |
		(static_cast<unsigned int>(buf[29]) << 16) |
		(static_cast<unsigned int>(buf[28]) << 8) |
		static_cast<unsigned int>(buf[27]);
	myRawDelay =
		(static_cast<unsigned int>(buf[34]) << 24) |
		(static_cast<unsigned int>(buf[33]) << 16) |
		(static_cast<unsigned int>(buf[32]) << 8) |
		static_cast<unsigned int>(buf[31]);
	myDelay = myRawDelay / 1000.0;
	myCycleCount = (static_cast<unsigned int>(buf[36]) << 8) | static_cast<unsigned int>(buf[35]);
	myRawTemperature = (static_cast<unsigned int>(buf[38]) << 8) | static_cast<unsigned int>(buf[37]);
