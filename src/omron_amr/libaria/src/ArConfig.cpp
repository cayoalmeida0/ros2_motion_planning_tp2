...
AREXPORT ArConfig::ArConfig(const ArConfig &config) : myRobotName(""),
                                                      myConfigName(""),
                                                      myLogPrefix("ArConfig: "),
                                     ...
                                     size_t errorBufferLen)
{
       ...
       sectionIt++)
  {
    ...
    if (ArUtil::strcasecmp(section->getName(), arg->getFullString()) == 0)
    {
      ...
      { // section is valid but shouldn't be parsed
    ...
    if (paramList != NULL)
    {
           ...
           pIter++)
      {
             ...
             parseIter++)
        {
          ...
          { // factory parameter and no permission to change
 ...
 **/
AREXPORT std::list<std::string> ArConfig::getSectionNamesInCategory(const char *categoryName) const
{
  std::list<std::string> retList;
  ...
  { // find sections that have no category
...
AREXPORT ArConfigSection::ArConfigSection(const char *name,
                                          const char *comment,
                                          bool isQuiet,
                                          const char *categoryName) : myName((name != NULL) ? name : ""),
 ...
 * if not found
 **/
AREXPORT ArConfigArg *ArConfigSection::findParam(const std::list<std::string> &paramNamePath,
                                                 bool isAllowHolders)
{
...
AREXPORT bool ArConfigSection::containsParamsOfPriority(ArPriority::Priority highestPriority,
                                                        ArPriority::Priority lowestPriority)
{
